...
describe("DesktopAssets", () => {
  ...
  it.effect("uses Nilox source-tree icons for unpackaged runs", () =>
    Effect.gen(function* () {
      const niloxEnvironmentLayer = DesktopEnvironment.layer({
        dirname: "/repo/apps/desktop/dist-electron",
        homeDirectory: "/Users/alice",
        platform: "darwin",
        processArch: "arm64",
        appVersion: "1.2.3-pi.1",
        appPath: "/repo",
        isPackaged: false,
        resourcesPath: "/repo/apps/desktop/resources",
        runningUnderArm64Translation: false,
      ...
      const fileSystemLayer = FileSystem.layerNoop({
        exists: (path) => Effect.succeed(String(path).includes("/assets/nilox/")),
      });
      const assets = yield* DesktopAssets.DesktopAssets.pipe(
        Effect.provide(
          DesktopAssets.layer.pipe(
            Layer.provide(Layer.merge(fileSystemLayer, niloxEnvironmentLayer)),
          ),
        ),
      );
      ...
      assert.match(Option.getOrThrow(icons.ico), /assets\/nilox\/t3-nilox-windows\.ico$/);
      assert.match(Option.getOrThrow(icons.png), /assets\/nilox\/nilox-macos-1024\.png$/);
      assert.isTrue(Option.isNone(icons.icns));
    }),
  );
