...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("uses the Nilox state root and Linux window class during early startup", () => {
    const input = {
      env: { T3CODE_DESKTOP_IDENTITY: "nilox" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path: string) => {
        assert.equal(path, "/home/user/.t3-nilox/userdata/desktop-settings.json");
        return JSON.stringify({ linuxPasswordStore: "kwallet6" });
      },
    };

    assert.deepEqual(resolveEarlyLinuxElectronOptions(input), {
